...
Component({
    ...
    */
	...
	ready(){
    this.setData({
      currentYear: this.data.beginYear,
      currentMonth: this.data.beginMonth
    });
		this.getAllArr();
    let beginDate = new Date(this.data.beginYear, this.data.beginMonth-1,this.data.beginDay);
    let endDate = new Date(this.data.endYear,this.data.endMonth-1,this.data.endDay);
    for (let i = 0; i < this.data.allArr.length; i++) {
      let date = this.data.allArr[i].fullDate;
      
      if (date.getTime() == beginDate.getTime()){
        this.setSelectDate(i, 1);
      }
      if (date.getTime() == endDate.getTime()){
        this.setSelectDate(i, 2);
      }
    }
	...
	methods: {
			...
			if (currentMonthDateLen > 0) {
				for (let i = 1; i <= currentMonthDateLen; i++) {
					currentMonthDateArr.push({
						...
						date: i,
            fullDate: new Date(this.data.currentYear, this.data.currentMonth-1,i),
            selected: false,
            remark:null
					})
				}
      ...
      let { year, month } = this.nextMonth(this.data.currentYear, this.data.currentMonth) // 获取上月 年、月
    ...
    setSelectDate(index,type){
      
      if (type==1){
        //入住
        for (var i = 0; i < this.data.allArr.length;i++){
          if (this.data.allArr[i].selected){
            this.data.allArr[i].selected =false;
          }
        }
        this.data.allArr[index].selected = true;
        this.data.allArr[index].remark = '入住';
      } else if (type == 2){
        this.data.allArr[index].selected = true;
        this.data.allArr[index].remark = '离店';
      }
      this.setData({
    ...
    tapDate(e){
      ...
      let obj ={
        index: i,
        item: this.data.allArr[i]
      }

      this.triggerEvent('tapDate', obj);
    }
	}
})